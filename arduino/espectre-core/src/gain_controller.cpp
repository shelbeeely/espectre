...
void GainController::process_packet(const wifi_csi_info_t* info) {
  ...
  if (packet_count_ < CALIBRATION_PACKETS) {
    ...
    agc_gain_locked_ = espectre::calculate_median_u8(agc_samples_, CALIBRATION_PACKETS);
    fft_gain_locked_ = espectre::calculate_median_i8(fft_samples_, CALIBRATION_PACKETS);
